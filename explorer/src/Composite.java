...
public class Composite implements Component {
	...
	@Override
	public Component getChild(int i) throws UnsupportedOperationException {//인자를 이용해 i번째 자식 가져오기
	...
	public String getValue() {//value getter
		String values = "";//값을 저장할 변수 선언
		for(int j = 0 ; j < components.size(); j++) {//자식 요소 갯수 만큼 반복
			Component component = components.get(j);//요소 객체에 j번째 자식요소를 저장
			values +=  component.getTag() + component.getValue();//해당 자식요소의 태그와 값을 누적
		}
		return values;//최종 누적 값을 리턴
	}

	@Override
	public void print() {//선택한 요소가 파일일 때 출력
		System.out.println(tag + this.getValue());//태그와 값을 출력
	}
	...
	public void print(int i) {//선택한 요소가 디렉토리일 때 출력
		...
		System.out.println("D: " + tag);//디렉토리 표시와 태그 출력
		for(int j = 0 ; j < components.size(); j++) {//자식 요소들의 갯수만큼 반복
			Component component = components.get(j);//현재 자식 요소를 가져옴 
			System.out.print(j + 1 +". ");//j.형태의 숫자 출력
			component.print(i + 1);//마지막 요소 출력
		}
	}
}
